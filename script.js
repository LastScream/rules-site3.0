...
(function () {
  ...
  function systemIsDark() { return matchMedia('(prefers-color-scheme: dark)').matches; }
  function effective(mode) { return mode === 'auto' ? (systemIsDark() ? 'dark' : 'light') : mode; }
  function renderThemeButton(mode) {
    if (!themeToggle) return;
    const eff = effective(mode);
    themeToggle.textContent = eff === 'dark' ? '🌙' : eff === 'light' ? '☀️' : '🌓';
    themeToggle.title = `Тема: ${mode}`;
  }
  function applyTheme(mode) { localStorage.setItem('theme', mode); root.setAttribute('data-theme', effective(mode)); renderThemeButton(mode); }
  applyTheme(getTheme());
